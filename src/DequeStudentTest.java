...
public class DequeStudentTest {
    ...
    @Before // run each time before test, constructor
    public void setup() {
        array = new ArrayDeque<>();
        linked = new LinkedDeque<>();
    }

    @Test(timeout = TIMEOUT) // individual unit test
    ...
    public void testArrayDequeNoWrapAround() {
        ...
        assertEquals(5, array.size()); //checking the size(expected value, the value of our classes)
        ...
        assertEquals("3a", array.removeLast()); // check return value
        ...
        expected[3] = null;
        expected[4] = null;
        assertArrayEquals(expected, array.getBackingArray()); // the array after removing
